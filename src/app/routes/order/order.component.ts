...
export class OrderComponent implements OnInit {
  constructor(private confirmServ: NzModalService, private fb: FormBuilder, private orderService: OrderService, private router: Router,
  private _message: NzMessageService, public _storage: SessionStorageService) {
  }
  ...
  submit(n): void {
    const url = ['接口地址1', '接口地址2', '接口地址3', '接口地址4']; //改为接口地址
    switch (n) {
      case 0: {
        for (let i = 1; i < 4; i++) {
          if (i == 2 && this.zhiyuan2 == false) continue;
          if (i == 3 && this.zhiyuan3 == false) continue;
          const weektemp = [], weekdaytemp = [], classNumtemp = [], coursetemp = [];
